...
public class ProductController {
    ...
    public ResponseBean findProductList(@RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                        ...
                                        ProductVO productVO) {
        ProductVO productVO1 = buildCategorySearch(categorys, productVO);
        PageVO<ProductVO> productVOPageVO = productService.findProductList(pageNum, pageSize, productVO1);
     ...
     * @param productVO
     */
    private ProductVO buildCategorySearch(@RequestParam(value = "categorys", required = false) String categorys, ProductVO productVO) {
        if (categorys != null && !"".equals(categorys)) {
            String[] split = categorys.split(",");
            switch (split.length) {
                case 1:
                    productVO.setOneCategoryId(split[0]);
                    break;
                case 2:
                    productVO.setOneCategoryId(split[0]);
                    productVO.setTwoCategoryId(split[1]);
                    break;
                case 3:
                    productVO.setOneCategoryId(split[0]);
                    productVO.setTwoCategoryId(split[1]);
                    productVO.setThreeCategoryId(split[2]);
                    break;
            }
        }
        return productVO;
    ...
    public ResponseBean edit(@PathVariable String id) {
        ProductVO productVO = productService.editProduct(id);
    ...
    public ResponseBean update(@PathVariable String id, @RequestBody ProductVO productVO) throws BusinessException {
        if (productVO.getCategoryKeys().length != 3) {
            throw new BusinessException(BusinessCodeEnum.PARAMETER_ERROR,"物资需要3级分类");
        }
        System.out.println(productVO.getImageUrl());
    ...
    public ResponseBean remove(@PathVariable String id) throws BusinessException {
    ...
    @PutMapping("/back/{id}")
    public ResponseBean back(@PathVariable String id) throws BusinessException {
    ...
    public ResponseBean publish(@PathVariable String id) throws BusinessException {
